exports.output = async ({message, args}) => {
    ...
    if (!/^#?([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(args[0])) {
        return Mike.exec.error(message, `The color must be in the hex format.`)
    }
    const color = resolveColor(args[0]);
    return Mike.exec.snap(message, `Hex: \`#${color.hex}\`\nRGB: \`rgb(${color.red}, ${color.green}, ${color.blue})\`\nHSL: \`hsl(${color.hue}, ${color.saturation}%, ${color.lightness}%)\``, false, null ,`http://placehold.it/500/${color.hex}/${color.hex}`)
}
